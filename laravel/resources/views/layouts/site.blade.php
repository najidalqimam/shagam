...
<body class="min-h-screen bg-bg text-ink antialiased">
    <header class="sticky top-0 z-50 border-b border-white/10 bg-brand/95 backdrop-blur">
        <div class="section-pad mx-auto grid h-16 max-w-6xl grid-cols-[1fr_auto_1fr] items-center gap-3">
            <a href="{{ route('home') }}" class="justify-self-start font-bold">
                <span class="block text-lg leading-none">{{ $settings['siteName'] ?? 'شاغم' }}</span>
                <span class="mt-1 hidden text-[0.65rem] text-mint sm:block">{{ $settings['tagline'] ?? '' }}</span>
            </a>
            ...
            <div class="flex items-center justify-end gap-2 justify-self-end">
                ...
                <a href="{{ route('join-operator') }}" class="hidden rounded-full border border-white/40 px-4 py-2 text-sm font-semibold lg:inline-flex">{{ $locale === 'ar' ? 'انضم كمشغّل' : 'Join as operator' }}</a>
                <a href="{{ route('request-service') }}" class="rounded-full bg-sand px-4 py-2 text-sm font-semibold text-ink-dark">{{ $locale === 'ar' ? 'اطلب خدمة' : 'Request a service' }}</a>
            </div>
        </div>
    </header>
